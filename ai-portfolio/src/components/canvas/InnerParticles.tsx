...
export default function InnerParticles({
  ...
  const [positions, sizes, speeds, offsets] = useMemo(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
      // Extremely tight constraint near the core beam
      p[i * 3 + 0] = (Math.random() - 0.5) * 0.05; 
      p[i * 3 + 1] = (Math.random() - 0.5) * 4.0; // full height
      p[i * 3 + 2] = (Math.random() - 0.5) * 0.05;

      s[i] = Math.random() * 10 + 5; // Smaller size for inner energy
      sp[i] = Math.random() * 1.5 + 0.8; // Faster speeds than dust
      o[i] = Math.random() * 100.0;
    }
    return [p, s, sp, o];
  }, []);
